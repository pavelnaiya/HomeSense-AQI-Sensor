...
2. Update the `FIRMWARE_VERSION` constant (e.g., from `"1.0.4"` to `"1.0.5"`):

   ```cpp
   #define FIRMWARE_VERSION "1.0.5"
   ```

   **Note:** All configuration, including firmware version and GitHub OTA settings, is now centralized in `include/config.h` for easier maintenance.
